...
import { WorkOrderOperationDetailsPageNameToOpen } from '../../../../SAPAssetManager/Rules/WorkOrders/Operations/Details/WorkOrderOperationDetailsPageToOpen';
import SmartFormsConfirmationLibrary from '../../../../SAPAssetManager/Rules/Forms/SmartFormsConfirmationLibrary';
...
/*
 * This SSAM class was overridden to enhance existing functionality related to
 * "completing" an Operation. Logic was added to ensure that if the operation
 * is "form-enabled", then the associated form has "mobile complete" status
 * before the operation can be completed/confirmed.
 *
 * Forms-related modification summary
 * - Function completeOperationFormsCheck() was added to ensure that a Mirata
 *   form associated with a Mirata form-enabled work order operation has been
 *   completed before the operation itself can be completed/confirmed. This
 *   function is now called from existing function completeOperation().
 * - The "libForms" import statement was added.
 */
export default class {
    ...
    static async completeOperation(context, mobileStatus, completeFunc) {
        ...
        // completeOperationFormsCheck() returns true if...
        //     1. The operation is Mirata form-enabled and the form is mobile complete, or
        //     2. The operation is not Mirata form-enabled
        // ...and allows the SSAM operation completion logic to proceed
        // Otherwise, it returns false to force a mobile status rollback
        const result = await this.completeOperationFormsCheck(context, binding);
        if (result) {
            ...
            // The Operation is Mirata form-enabled but the associated form is not
            // "mobile complete"
            return Promise.reject(); // Reject to force a mobile status rollback
        }
    }
    ...
    static unconfirmOperation(context) {
        ...
        return this.showUnconfirmOperationWarningMessage(context).then(
            async doMarkUnconfirm => {
                ...
                let confirmation = await context.read('/SAPAssetManager/Services/AssetManager.service', 'Confirmations', [], `$filter=OrderID eq '${binding.OrderId}' and Operation eq '${binding.OperationNo}' and SubOperation eq '' and sap.islocal() and FinalConfirmation eq 'X'&$orderby=ConfirmationCounter desc&$top=1`).then(function(confirmationResults) {
    ...
    static createBlankConfirmation(context, sOffset = 0, item = undefined) {
        ...
        return Promise.all([ConfirmationNum, ConfirmationCounter]).then((resolvedValues) => {
            let binding = WorkOrderCompletionLibrary.getInstance().getBinding(context) || context?.getPageProxy()?.getActionBinding();
            if (item) {
                binding = item;
            }
            let odataDate = new ODataDate();
    ...
    /**
     * Checks if a form-enabled operation's associated form is "mobile complete"
     * before allowing the operation to be completed/confirmed. If the form is
     * not "mobile complete", a warning dialog is displayed and the operation
     * completion/confirmation is prevented.
     *
     * @param {IClientAPI} context - The client context object
     * @param {Object} binding - The binding object containing operation details
     * @returns {Promise<boolean>} Returns true if:
     *   1. The operation is not form-enabled, or
     *   2. The operation is form-enabled and its associated form is "mobile complete"
     *   Returns false if the operation is form-enabled but its form is not "mobile complete"
     */
    static async completeOperationFormsCheck(context, binding) {
        const isOperationFormEnabled = await libForms.isBusinessObjectFormEnabled(context, binding);
        if (isOperationFormEnabled) {
            const isMobileComplete = await libForms.isLatestFormSubmissionMobileCompleteForOperation(context, binding);
            if (!isMobileComplete) {
                // The Operation is form-enabled but the form is not
                // "mobile complete". Inform the user of this fact
                // and return false to prevent the Operation form
                // being completed/confirmed.
                await libForms.showWarningCompleteFormBeforeConfirmOperation(context);
                return false;
            } else {
                // The Operation is form-enabled and the form is
                // "mobile complete", so return true
                return true;
            }
        } else {
            // The Operation is not form-enabled, so return true since no
            // Mirata form is involved.
            return true;
        }
    }
}
