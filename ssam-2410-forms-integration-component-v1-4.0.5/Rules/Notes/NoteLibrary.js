...
import { SubOperationDetailsPageName } from '../../../SAPAssetManager/Rules/SubOperations/SubOperationDetailsPageToOpen';
import { NotificationDetailsPageName } from '../../../SAPAssetManager/Rules/Notifications/Details/NotificationDetailsPageToOpen';
import { ServiceOrderDetailsPageName } from '../../../SAPAssetManager/Rules/ServiceOrders/ServiceOrderDetailsPageToOpen';
import { ServiceItemDetailsPageName } from '../../../SAPAssetManager/Rules/ServiceOrders/ServiceItems/ServiceItemDetailsPageToOpen';
import IsOnlineNotificationItem from '../../../SAPAssetManager/Rules/OnlineSearch/Notifications/IsOnlineNotificationItem';

/**
 * This SSAM class was overridden to implement logic for adding text originating
 * from a Mirata form to the notes property of the work order operation
 * currently in context. The SSAM Note handling logic is a based on the mobile
 * user using a UI screen to enter the note text to add. However, the Mirata
 * Forms note-based logic does not require nor rely on a UI screen to compile
 * the note text to add. In order to leverage the complex SSAM Note handling
 * logic, a "virtual" Forms-based note text input screen was defined. The Forms-
 * based note text to add is then injected into the SSAM Note handling logic as
 * if it were collected by the "virtual" screen.
 *
 *
 * Performing a search for the text "mirata" will show all the modification
 * points made to this class when compared to the original SSAM class.
 */
export class NoteLibrary {
    ...
    static noteDownload(pageClientAPI, completionEntitySet, service = '/SAPAssetManager/Services/AssetManager.service') {
        let note;
        // Note: For Mirata Forms integration, ensure the required context data
        // exists for retrieving the note text when the target business object
        // is a work order operation. It was determined that the state variable
        // "TransactionNoteType" is not set when the source of this function call
        // originates from an object card whose context is a work order operation.
        if (!pageClientAPI.getAppClientData().StateVariables.TransactionNoteType) {
            if (!this.didSetNoteTypeTransactionForBindingType(pageClientAPI)) {
                throw new Error('Unable to set TransactionNoteType state variable');
            }
        }
    ...
    /**
     * Download existing note text for a business object using a custom binding.
     * The custom binding must have the following properties:
     * - @odata.type: The OData entity type of the business object (string)
     * - @odata.id: The OData ID of the business object (string)
     *
     * This function is implemented to support the Mirata Forms integration.
     * Specifically, it is needed to support a case where multiple business
     * objects are being evaluated concurrently for form enablement, but in an
     * asynchronous manner. The "noteDownloadValue" function relies on one of
     * the following to retrieve the target business object via the "@odata.id"
     * property (evaluated in the order shown below):
     * - The context binding (context.binding)
     * - The action binding (context.getActionBinding())
     * Since multiple business objects were being evaluated concurrently in the
     * scenario mentioned above, the context binding option was not available as
     * the source for the "@odata.id" property. The action binding option was
     * tried, but it was found to be unreliable for retrieving the "@odata.id"
     * property because, unlike the context binding (which is immutable), the
     * context action binding is mutable and its internal data source appears
     * to be "global" in nature.
     *
     * @param {PageProxy} pageClientAPI - The page proxy object
     * @param {Object} customBinding - The custom binding object
     * @returns {Promise<String>} - The note text or null if no note is found
     * @throws {Error} - If the custom binding is not provided or does not
     *    have the required properties
     */
    static async noteDownloadValueByCustomBinding(pageClientAPI, customBinding) {
        if (!customBinding) {
            throw new Error('Custom binding is required');
        }
        if (!customBinding['@odata.type'] || !customBinding['@odata.id']) {
            throw new Error('Custom binding must have @odata.type and @odata.id properties');
        }
        // The following function call requires @odata.type to be set in the
        // custom binding object.
        if (!this.didSetNoteTypeTransactionForBindingType(pageClientAPI, customBinding)) {
            throw new Error('Unable to set TransactionNoteType state variable');
        }

        const service = '/SAPAssetManager/Services/AssetManager.service';
        const transactionTypeObject = libCommon.getStateVariable(pageClientAPI, ConstantsLibrary.transactionNoteTypeStateVariable);
        const entitySet = customBinding['@odata.id'] + '/' + transactionTypeObject.component
        const filter = this.getFilter(pageClientAPI); // Additional filtering is needed for some note types

        //Read the existing note
        const result = await pageClientAPI.read(service, entitySet, [], filter);
        if (!libVal.evalIsEmpty(result)) {
            const note = result.getItem(0);
    ...
    static didSetNoteTypeTransactionForBindingType(clientAPI, customBinding = undefined) {
        ...
        switch (bindingType.substr(startIndex)) {
            ...
            case 'S4ServiceQuotationItem':
                ...
                break;
            case 'MyWorkOrderOperationMirataFormsNote':
                this.setNoteTypeTransactionFlag(clientAPI, TransactionNoteType.workOrderOperationMirataFormsNote());
                break;
            default:
    ...
    static didSetNoteTypeTransactionFlagForPage(clientAPI, page) {
        switch (page) {
            ...
            case 'OnlineWorkOrderOperationDetailsWithObjectCards':
                ...
                break;
            case 'WorkOrderOperationMirataForm':
                this.setNoteTypeTransactionFlag(clientAPI, TransactionNoteType.workOrderOperationMirataFormNote());
                break;
            default:
                // Didn't set the transaction type
                return false;
...
export class TransactionNoteType {
    ...
    static serviceConfirmationItemLongText(clientAPI) {
        return {
            ...
            name: 'S4ServiceConfirmationItem_Nav',
            entitySet: 'S4ServiceConfirmationItems',
            entitySetLink: `S4ServiceConfirmationItems(ObjectID='${clientAPI.binding.ObjectID}',ObjectType='${clientAPI.binding.ObjectType}',ItemNo='${clientAPI.binding.ItemNo}')`,
            longTextEntitySet: 'S4ServiceConfirmationLongTexts',
            page: 'NotesListView',
            noteUpdateAction: '/SAPAssetManager/Actions/Notes/Update/NotesUpdateOnServiceItem.action',
            noteDeleteAction: '/SAPAssetManager/Actions/Notes/Delete/NotesDelete.action',
    ...
    static workOrderOperationMirataFormNote() {
        return {
          component: 'OperationLongText',
          name: 'WorkOrderOperation',
          entitySet: 'MyWorkOrderOperations',
          longTextEntitySet: 'MyWorkOrderOperationLongTexts',
          page: 'FormsExtensionControlPage',
          noteCreateAction: '/MirataFormsIntegrationComponents/Actions/Forms/Notes/NotesCreateOnWOOperationForMirataForm.action',
          noteUpdateAction: '/MirataFormsIntegrationComponents/Actions/Forms/Notes/NotesUpdateOnWOOperationForMirataForm.action',
          noteDeleteAction: ''
        };
    }
}
