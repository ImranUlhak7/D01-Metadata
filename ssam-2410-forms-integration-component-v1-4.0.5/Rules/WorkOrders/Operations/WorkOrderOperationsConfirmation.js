...
import CreateLinks from '../../../../SAPAssetManager/Rules/Confirmations/CreateUpdate/OnCommit/CreateLinks';
...
import { AddBulkConfirmationAction, RunNextBulkConfirmationAction } from '../../../../SAPAssetManager/Rules/WorkOrders/Operations/BulkConfirmationQueue';
import ConfirmationsIsEnabled from '../../../../SAPAssetManager/Rules/Confirmations/ConfirmationsIsEnabled';
import TimeSheetCreateUpdateCreateLinks from '../../../../SAPAssetManager/Rules/TimeSheets/CreateUpdate/TimeSheetCreateUpdateCreateLinks';
import { CreateBulkConfirmationSignature, ResetBulkConfirmationSignatureFlow } from '../../../../SAPAssetManager/Rules/WorkOrders/Operations/BulkConfirmationLibrary';
import { checkMeterComponentBeforeCompletion } from '../../../../SAPAssetManager/Rules/WorkOrders/Complete/FinalizeCompletePageMessage';
...
export default function WorkOrderOperationsConfirmation(context) {
    ...
    for (let i = 0; i < operationsConfirmations.length; i++) {
        ...
        // If the operation is form-enabled and the associated form has not been completed,
        // do not confirm the operation. Otherwise, continue with the operation confirmation
        // process.
        promiseArr.push(completeOperationFormsCheck(context, item).then((result) => {
            if (result) {
                return checkMeterComponentBeforeCompletion(context, item).then((result) => {
                    if (result.data) {
                ...
                item.error = 'The form associated with the operation has not been completed';
                failedOperations.push(item);
                return Promise.resolve();
            }
        }));
...
/**
 * Checks if operation is form-enabled, and if so, checks if the associated
 * form has "mobile complete" status.
 * @param {IClientAPI} context - The page context object
 * @param {object} operation - The operation object
 * @returns {Promise<boolean>} - Returns false if the operation is form-enabled
 *    and the associated form does not have "mobile complete" status; otherwise,
 *    returns true
 */
async function completeOperationFormsCheck(context, operation) {
  let result = await libForms.isBusinessObjectFormEnabled(context, operation);
  if (result) {
      // If the operation is form-enabled, then check if the associated form has
      // "mobile complete" status.
      result = await libForms.isLatestFormSubmissionMobileCompleteForOperation(context, operation);
  } else {
      // If the operation is not form-enabled, then return true so that the
      // operation confirmation process can continue.
      result = true;
  }
  return result;
}
