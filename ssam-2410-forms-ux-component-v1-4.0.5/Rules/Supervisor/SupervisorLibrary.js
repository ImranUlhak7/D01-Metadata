...
export default class {
     ...
     * @param {*} context
     */
    static isBusinessObjectEditable(context, binding = context.binding) {
        ...
        if (libThis.isSupervisorFeatureEnabled(context)) {
            return libThis.isUserTechnician(context).then(tech => {
                if (tech) {
                    ...
                    } else if ((assignmentType === '6' || assignmentType === '2') && type === '#sap_mobile.MyWorkOrderOperation') { //Operation work center or phase model
                        const mobileStatus = libMobile.getMobileStatus(context.binding, context);
                        if (binding.PersonNum === person || ((!binding.PersonNum || binding.PersonNum === '00000000') && (mobileStatus === 'RECEIVED'))) {
    ...
    static getFilterForWOPendingReview(context, useDataQuery = true, useFilter = false) {
        ...
        if (useDataQuery) {
            ...
            queryBuilder.expand('WODocuments,WODocuments/Document,OrderMobileStatus_Nav,Operations,Operations/SubOperations,Operations/Tools,WOPriority,MarkedJob,UserTimeEntry_Nav,WOPartners,WOPartners/Employee_Nav');
        ...
        '&$expand=WODocuments,WODocuments/Document,OrderMobileStatus_Nav,Operations,Operations/SubOperations,Operations/Tools,WOPriority,MarkedJob,UserTimeEntry_Nav,WOPartners,WOPartners/Employee_Nav' +
